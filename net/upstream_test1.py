...
class UpstreamTest1(PreTrainedModel):
    ...
    def __init__(self, config, pretrained_path = None,device = None):
        ...
        # Gender dimension added to input
        input_dim = self.upstream.config.hidden_size + 3  # +3 for gender one-hot
    ...
    def forward(self, x, sr, gender_onehot):
        ...
        # Concatenate gender with pooled audio features
        combined_features = torch.cat([pooled_features, gender_onehot], dim=-1)
        
        # Pass through classifier and continuous projection
        category, ed = self.classifier(combined_features, return_embedding=True)
        ec = self.cont_proj(combined_features)
