...
class UpstreamTest2(PreTrainedModel):
    ...
    def __init__(self, config, pretrained_path = None,device = None):
        ...
        # Gender embedding projection
        self.gender_proj = nn.Sequential(
            nn.Linear(3, config.hidden_dim // 2),  # Project to smaller dim first
            nn.Tanh(),
            nn.Dropout(config.dropout)
        )
        
        # Modified regressor to handle gender-augmented embeddings
        self.regressor = HierarchicalDCRegressionHead(
            classifier_embed_dim=config.hidden_dim + config.hidden_dim // 2,  # ed + gender
            cont_embed_dim=config.hidden_dim + config.hidden_dim // 2,       # ec + gender
    ...
    def forward(self, x, sr, gender_onehot):
        ...
        # Get gender embedding
        gender_embed = self.gender_proj(gender_onehot)
        
        # Fuse gender with both embeddings
        ed_with_gender = torch.cat([ed, gender_embed], dim=-1)
        ec_with_gender = torch.cat([ec, gender_embed], dim=-1)
        
        # Use fused embeddings for regression
        dim = self.regressor(ed_with_gender, ec_with_gender)
